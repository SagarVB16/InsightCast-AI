...
def scrape_with_firecrawl(url: str) -> str:
    ...
    if not api_key:
        raise HTTPException(status_code=500, detail="Firecrawl API key not found in environment variables.")
    ...
    try:
        response = requests.post("https://api.firecrawl.dev/v2/scrape", json=payload, headers=headers)
        response.raise_for_status()
        data = response.json()
        
        if not data.get("success"):
            error_msg = data.get("error", "Unknown Firecrawl error")
            raise HTTPException(status_code=500, detail=f"Firecrawl error: {error_msg}")
            
        return data.get("data", {}).get("html", "")
    except requests.exceptions.RequestException as e:
        raise HTTPException(status_code=500, detail=f"Firecrawl request failed: {str(e)}")
...
def summarize_with_groq_news_script(api_key: str, headlines: str) -> str:
    """
    Summarize multiple news headlines into a TTS-friendly broadcast news script using Groq models via LangChain.
    """
    system_prompt = """
...
"""
    ...
    try:
        ...
        # Invoke Groq LLM with system + user prompt
        response = llm.invoke([
            SystemMessage(content=system_prompt),
